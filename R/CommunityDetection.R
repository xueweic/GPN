...
#'
#' @param BiNet M by K matrix of the signed bipartite genotype and phenotype network, row denotes the M SNPs and column denotes K phenotypes.
...
#' @param quiet decide if show the perturbation times. default: FALSE (show procedure)
#'
#' @return A list of ClusterNumber: the optimal number of clusters (C); Clusters: K by C matrix of the optimal cluster results; AF: the area under the curve of CDF calculated from the perturbation results.
#' @export
#'
#' @examples
#' M <- 100
...
#'
#'
CommunityDetection <- function(BiNet, npert = 100, mMax = "None", cluster.method = "ward.D2", quiet = FALSE)
{
  ...
  if (!hasArg(BiNet)){
    ...
    if (sum(!is.finite(BiNet)) != 0){
      temp <- fortify.zoo(BiNet) %>% mutate_all(function(x) ifelse(!is.finite(x), 0, x))
      BiNet <- as.matrix(temp)[,-1]
    } else {
      BiNet <- as.matrix(BiNet)
    }
  }
  ...
  if (mMax == "None"){
    ...
    if (mMax > ncol(BiNet)){
      stop("Error: The maximum searching number of clusters must smaller than the number of phenotypes!")
    }
  }
  ...
  sigma2 <- sqrt(median(apply(BiNet,1,var)))*1
  ...
  for(k in 1:npert)
  {
    ypert <- BiNet + matrix(sigma2*rnorm(n*m),n,m)
    Sigma <- cor(ypert)
    dist <- 1-Sigma
    hc=hclust(as.dist(dist), method = cluster.method)
    for(i in 2:(mMax-1))
    ...
    if (!quiet){print(paste("Perturbation times:", k, "among", npert, "!"))}
  }
  for(i in 2:(m-1))
  {
    Sp[[i-1]] <- Sp[[i-1]]/npert
    D[[i-1]] <- abs( S[[i-1]]-Sp[[i-1]] )
    for(k in 1:100) AF[i-1] <- AF[i-1] + sum((D[[i-1]]<=(k-0.5)/100))
  }
  L.max <- length(AF)
  ...
  khat <- IND
  Bb <- B[[khat-1]]
  return(list("ClusterNumber" = khat,
              "Clusters" = Bb,
              "AF" = AF))
}
